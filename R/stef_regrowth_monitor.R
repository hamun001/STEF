...
stef_regrowth_monitor <- function(inraster,my_dates, mYear, spatiaNormPercentile,threshold, densityPlot,windowwidth,tryCatchError,sPatioNormalixse,minimumRegrowthTime, ...) {
  library("raster")
  library("rgdal")
  library("spatial.tools")
  require("doParallel")
  
  spatioTemporalMetricsExtractorxc_regrowth_monitor <- function(dFrvx,my_dates, mYear,spatiaNormPercentile,threshold, densityPlot,windowwidth,sPatioNormalixse, minimumRegrowthTime){
    dFrv <- dFrvx
            ...
            if (length(xdatamx$x) > 1){
              ...
              while ( is.na(chan) & countx < (length(xdatamx$x) - 2)){
                ...
                if (xdatamx$x[countx] < qt & xdatamx$x[sxz1] < qt){
                  ...
                  if (diff_distarbaDate_LastDate > minimumRegrowthTime){
  ...
  samo <- c(rep(NA, 17))
  if(tryCatchError){
    result <-  tryCatch (spatioTemporalMetricsExtractorxc_regrowth_monitor(dFrvx,my_dates, mYear,spatiaNormPercentile,threshold, density,windowwidth,sPatioNormalixse,minimumRegrowthTime ),
    ...
    result <- spatioTemporalMetricsExtractorxc_regrowth_monitor(dFrvx,my_dates, mYear,spatiaNormPercentile,threshold, density,windowwidth,sPatioNormalixse,minimumRegrowthTime)
  }
  return(result)
}
